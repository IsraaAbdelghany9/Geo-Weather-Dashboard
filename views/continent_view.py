...
def render_continent_view(df: pd.DataFrame):
    if df is None or df.empty:
        return html.Div("No weather data available. Select a continent and click 'Get Weather'.", style={"color": "red"})
    ...
    fig_bar = px.bar(
        ...
        x="country",
        y="temp_c",
        title=f"Average Temperature (°C) by Country in {selected_continent}",
        labels={"temp_c": "Average Temperature (°C)", "country": "Country"},
        color="temp_c", # Color bars by temperature
    ...
    return html.Div([
        html.H4(f"Weather in {selected_continent}", className="mb-3 text-center"),
